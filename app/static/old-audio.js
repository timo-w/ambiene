...
function playRandomNote(notesList, wave='sine', gain=0.5) {
    ...
    webSocket.send(JSON.stringify({type: "audio_message", message: {note: randomNote, wave: wave, gain: gain}}));
}
...
for (let i=0; i<keyGrid.length; i++) {
    keyGrid.item(i).addEventListener("click", () => {
        setTimeout(() => {
            webSocket.send(JSON.stringify({type: "audio_message", message: {note: keyGrid.item(i).value, wave: 'sawtooth', gain: 0.15}}));
